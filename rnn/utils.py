...
class mlp_gasopt_inlined_processing(nn.Module):
    ...
    def __init__(self, device, 
        ...
        SLINGO_BOUNDS = [0, 4200, 8000, 14286, 50000]      # wavenumber edges of Slingo bands
        SLINGO_TO_COEFFS = [3, 2, 1, 0]                     # Slingo wavenum-index -> coeffs index

        # slingo_weights: (num_bands, 4) — weight of each Slingo coeffs index for each custom band
        slingo_weights = torch.zeros(self.num_bands, 4, dtype=torch.float32)
        for b in range(self.num_bands):
            wlo_band = float(self.wavenum_bounds[b])
            whi_band = float(self.wavenum_bounds[b + 1])
            total_overlap = 0.0
            for s in range(4):
                wlo_s = float(SLINGO_BOUNDS[s])
                whi_s = float(SLINGO_BOUNDS[s + 1])
                overlap = max(0.0, min(whi_band, whi_s) - max(wlo_band, wlo_s))
                if overlap > 0.0:
                    coeffs_idx = SLINGO_TO_COEFFS[s]
                    slingo_weights[b, coeffs_idx] += overlap
                    total_overlap += overlap
            if total_overlap > 0.0:
                slingo_weights[b] /= total_overlap
        print("slingo weights:", slingo_weights)
        self.register_buffer('slingo_weights', slingo_weights)  # (num_bands, 4)
...
def load_reduced_gas_optics_model(
    ...
    # wavenum_bounds = ckpt.get("wavenum_bounds", None)
    wavenum_bounds = rrtmgp_bounds_to_wavenum_bounds(rrtmgp_band_bounds)
  
    print("BAND BOUNDS", band_bounds, "RRTMGP BANDS", rrtmgp_band_bounds, "wavenum_bounds", wavenum_bounds)
...
class train_or_eval_one_epoch:
    def __init__(self, dataloader, 
                 ...
                 device, dtype,
                 cfg, 
                 metrics_det, metric_h_con, metric_wcon, metric_rh,
        ...
        self.metric_wcon = metric_wcon
        self.metric_rh = metric_rh
        ...
        if self.batch_size==384:
            self.epoch_bias_collev = np.zeros((384,  self.model.nlev,  self.ny_pp))
    ...
    def eval_one_epoch(self, lossf, optim, epoch, timesteps=1, lr_scheduler=None):
        ...
        running_loss = 0.0; running_energy = 0.0; running_water = 0.0; running_wcon = 0.0; running_wcon_true=0.0
        ...
        epoch_rmse_perlev = 0.0; epoch_prec_std_frac = 0.0; epoch_hum_std_ratio = 0.0
        epoch_prec_99p_ratio = 0.0; epoch_tend_99p_ratio = 0.0
        epoch_prec_99p_day = 0.0; epoch_prec_99p_hour = 0.0
        epoch_prec_50p_day = 0.0; epoch_accumprec = 0.0; epoch_qn_pos=0.0; epoch_qv_pos=0.0
        prec_true_daily = []; prec_pred_daily = []
        prec_true_hourly = []; prec_pred_hourly = []; precip_neg_mse = 0.0
            ...
            for ichunk in range(len(x_lay_chk)):
                ...
                if self.cfg.do_semi_online_training:
                    if j>100:
                        ...
                        # to work out what the dynamics contribution was:
                        #  X_pred[k] = X_pred[k-1] + dX_phys_nn[k-1] + dX_dyn_k,
                        #       where the dynamics contribution
                        #  dX_dyn_k = (X_true[k] - X_true[k-1]) - dX_phys_true[k-1]
                        #           = (X_true[k] - X_true[k-1]) - y[k-1]
                            
                        #   NN is used autoregressively for k2 timesteps (rollout window)
                        #       dX_phys_nn[k], ysfc_phys_nn = NN(X_pred[k], L.S. forcings, surface inp., hidden memory)
                        #   until k=k2 then reset:
                        #      X_pred = X_true     
                        #      dX_phys = dX_phys_true 
                        # 
                    ...
                    if self.cfg.autoregressive:
                      ...
                      if self.cfg.predict_fluxes:
                        if self.model.return_neg_precip:
                          ...
                          # print("mean max prec neg", torch.mean(precip_negative).item(), torch.max(precip_negative), "mse", precip_neg_mse.item())
                ...
                if (not self.cfg.autoregressive) or (self.cfg.autoregressive and (j+1) % timesteps==0):
                    ...
                    with torch.autocast(device_type=device.type, dtype=self.dtype, enabled=enable_autocast):#self.cfg.mp_autocast):
                        ...
                        wcon       = torch.mean(torch.square(wcon_p - wcon_t))
                        cloudpath_err   = torch.mean(torch.square(cld_pred - cld_tot))
                        del wcon_p, wcon_t, cld_pred, cld_tot
                        wcon_p_long     = self.metric_wcon(ypo_lay, ypo_sfc, surf_pres_denorm, lhf, x_lay_raw, timesteps)
                        wcon_t_long     = self.metric_wcon(yto_lay, yto_sfc, surf_pres_denorm, lhf, x_lay_raw, timesteps)#,printdebug=True)
                        wcon_long       = timesteps*torch.mean(torch.square(wcon_p_long - wcon_t_long))
                        running_wcon += torch.mean(wcon_p_long).detach().cpu().numpy()
                        running_wcon_true += torch.mean(wcon_t_long).detach().cpu().numpy()
                    ...
                    if self.train:
                        ...
                        if self.cfg.use_precip_gel_loss:
                            losses.append(self.cfg.w_precgel*precip_sum_gel)
                    ...
                    if j>loss_update_start_index:
                        with torch.no_grad():
                            ...
                            if self.model_is_stochastic:
                                ...
                                epoch_q_err_corr += np.corrcoef(err_dq_ens1,err_dq_ens2)[0,1]**2
                            ...
                            for ip in range(self.ny_pp):
                                ...
                                std_ratios[ip] = np.std(tend_pred) / np.std(tend_true)
                    ...
                    if self.cfg.autoregressive:
                        ...
                        if self.cfg.gradual_mixing_end_epoch != 0: # enable gradual mixing
                            ...
                            # ep=10 --> 20% of convective memory should be previous predictions, 80% should be previous true tendencies
                            # ep=50 --> 100% should be previous predictions
                            mixing_ratio = min(epoch/self.cfg.gradual_mixing_end_epoch,1.0)
                            nrandom = int(mixing_ratio * self.batch_size)
                        else:
        ...
        if self.cfg.mp_mode != -2:
            self.metrics["qn_pos"] = epoch_qn_pos / k
        ...
        if (self.replay == "mixed") and (self.cfg.gradual_mixing_end_epoch != 0):
            print("Gradual mixing was on, {}% of previous tendencies used as convective memory came from predictions".format(100*mixing_ratio))
...
@njit(fastmath=True)    
def v4_to_v5_inputs_numba(x_lev_b, liq_frac, lbd_qn):
    ...
    for ii in range(ns):
        for jj in range(nlev):
            ...
            x_lev_b[ii,jj,2] = qn
            x_lev_b[ii,jj,3] = liq_frac[ii,jj]
...
class generator_xy(torch.utils.data.Dataset):
    def __init__(self, filepath, nloc=384, cache=False,
                 ...
                 include_q_input=False, # also include q in addition to rh
                 rh_prune=False,
                 output_prune=False,
                 mp_mode=0,
                 hybm=None,
        ...
        self.snowhice_fix = snowhice_fix
        self.qinput_prune = qinput_prune
        self.rh_input_to_q=rh_input_to_q
        self.include_q_input = include_q_input
        self.rh_prune = rh_prune 
        ...
        if self.mp_mode==-2:
            self.pred_cld_water_frac=True 
        else:
            self.pred_cld_water_frac=False
    ...
    def compute_liq_frac(self, T):
        liquid_frac = (T - 253.16) * 0.05 
    ...
    def __getitem__(self, indices):
        ...
        if self.apply_new_input_scaling:
            ...
            # print("min max q scaled", x_lev_b[:,:,-1].min(), x_lev_b[:,:,-1].max())
            if self.rh_input_to_q:
                if self.include_q_input:
                    q = x_lev_b[:,:,-1] 
                    q[q<0.0] = 0.0
        ...
        if self.hu_mp_constraint:
            ...
            if self.pred_cld_water_frac:
                qv_before   = x_lev_b_denorm[:,:,-1]
                dqv         = y_lev_b[:,:,1]  
                dqtot       = dqv + dqn

                qv_new      = qv_before + dqv*1200
                qv_new[qv_new<0.0] = 0.0
                qtot_new    = qv_new + qn_new 
                inds_pos    = qtot_new > 0

                tot_cld_frac = np.zeros(dqn.shape,dtype=np.float32)
                # print("min max mean qv old", qv_before.min(), qv_before.max(), qv_before.mean())
                # print("min max mean qv new", qv_new.min(), qv_new.max(), qv_new.mean())
                # print("min max mean qn new", qn_new.min(), qn_new.max(), qn_new.mean())
                # print("min max mean qtot new", qtot_new.min(), qtot_new.max(), qtot_new.mean())
                tot_cld_frac[inds_pos] = qn_new[inds_pos] / qtot_new[inds_pos]
                tot_cld_frac = np.sqrt(np.sqrt(tot_cld_frac))
                y_lev_b[:,:,1] = dqtot
                y_lev_b[:,:,2] = tot_cld_frac
                # print("min max mean tot_cld_frac", tot_cld_frac.min(), tot_cld_frac.max(), tot_cld_frac.mean())
            else:
                y_lev_b[:,:,2] = dqn
            y_lev_b[:,:,3] = liq_frac 

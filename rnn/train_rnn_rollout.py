...
tr_data_fname = ["data_v4_rnn_year2.h5", 
                "data_v4_rnn_year3.h5",
                "data_v4_rnn_year4.h5",
                "data_v4_rnn_year5.h5"]

tr_data_path = [data_dir + b for b in tr_data_fname]

tr_data_path = data_dir + "data_v4_rnn_nonorm_febtojan_year1.h5"
# tr_data_path = data_dir + "train_v4_rnn_nonorm_febtojan_y1-5.h5"

if use_val:
    val_data_fname = "data_v4_rnn_year8.h5"
...
if autoregressive:
    model = RNN_autoreg(cell_type=model_type, 
            nlay = nlev, 
            nx = nx, nx_sfc=nx_sfc, 
            ny = ny, ny_sfc=ny_sfc, 
            nneur=nneur,
...
if use_val:
    ...
    if type(val_data_path)==list:  
        ...
        chunk_size_val = chunksiz // num_files
    else:
        chunk_size_val = chunksiz # 10 days (3 time steps in an hour, 72 in a day)
...
train_data = generator_xy(tr_data_path, nloc=nloc, add_refpres=add_refpres, mp_mode=mp_mode,
...
if use_val:
    ...
    val_data = generator_xy(val_data_path, nloc=nloc, add_refpres=add_refpres, mp_mode=mp_mode,
                            ycoeffs=ycoeffs_gen, xcoeffs=xcoeffs_gen, ycoeffs_ref=ycoeffs_gen_ref, xcoeffs_ref=xcoeffs_gen_ref)
...
regular_loss = metrics.get_mse_flatten(weights)
                ...
                if (not self.autoregressive) or (self.autoregressive and (j+1) % timewindow==0):
                    ...
                    if mp_autocast:
                        with torch.autocast(device_type=device.type, dtype=dtype):
                            ...
                            mse = regular_loss(targets_lay, targets_sfc, preds_lay, preds_sfc)
                            
                            if use_mp_constraint:
                                ypo_lay, ypo_sfc = model.pp_mp(preds_lay, preds_sfc, x_lay_raw)
                                # with torch.no_grad(): yto_lay, yto_sfc = model.pp_mp(targets_lay, targets_sfc, x_lay_raw )
...
for i,testdata in enumerate(loader):
    x_lay_chk, x_sfc_chk, targets_lay_chk, targets_sfc_chk, x_lay_raw_chk, ytos_lay_chk, ytos_sfc_chk  = testdata

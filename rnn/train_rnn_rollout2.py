...
if use_val:
    val_data_fname = "data_v4_rnn_nonorm_year8.h5"
    val_data_dir = data_dir
    val_data_dir = "/media/peter/samsung/data/ClimSim_low_res_expanded/"
    nloc_val = 384 
...
hyam = torch.from_numpy(data.grid_info['hyam'].values).to(device)
hybm = torch.from_numpy(data.grid_info['hybm'].values).to(device)
